...
GERMAN_BASIC_VERBS = {
    "sein", "ist", "sind", "war", "gewesen", "habe", "haben", "hat", "hatte", "gehabt",
...
EXCLUDE_WORDS = {"mein", "meine", "meines", "meiner", "meinem", "meinen", "dein", "deine", "deines", "deiner", "deinem", "deinen", "sein", "seine", "seines", "seiner", "seinem", "seinen", "unser", "unsere", "unseres", "unserer", "unserem", "unseren", "euer", "eure", "eures", "eurer", "eurem", "euren", "ihr", "ihre", "ihres", "ihrer", "ihrem", "ihren", "dieser", "dieses", "diese", "diesem", "diesen", "dieser", "jener", "solcher", "welcher"}
...
def deepl_direct_translate(text, source_lang, target_lang):
    ...
    try:
        ...
        return response.json()["translations"][0]["text"] if response.status_code == 200 else f"Error: {response.status_code}"
    except Exception as e:
        return f"Error: {str(e)}"

def smart_translate(text, pos, source_lang):
    src = source_lang.lower()
    query = f"{text} (Verb)" if pos == "VERB" else (f"Idiom: {text}" if pos == "PHRASE" else text)
...
if st.button("开始深度解析") and sentence:
    ...
    for token in doc:
        ...
        if token.pos_ in ["VERB", "AUX", "ADJ", "ADV", "NOUN", "PROPN"]:
            ...
            if is_basic(lemma, original_text, token.pos_): continue
            ...
            if source_code == "de" and token.pos_ == "VERB":
                if lemma.endswith("een") and not lemma.endswith("gehen"): lemma = lemma.replace("een", "eten") if "et" in original_text.lower() else original_text.lower()
                if token.i in particles_map and not lemma.startswith(particles_map[token.i]): lemma = particles_map[token.i] + lemma

            cache_key = f"{lemma}_{token.pos_}"
            if cache_key not in processed_keys and not is_basic(lemma, lemma, token.pos_):
                row = {"词原形": lemma, "中文意思": smart_translate(lemma, token.pos_, source_code), "辅助解析": deepl_direct_translate(lemma, source_code, target_aux_code)}
