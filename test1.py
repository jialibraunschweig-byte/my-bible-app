...
if parse_btn and sentence:
    with st.spinner('安全加密通道一体化解析中...'):
        ...
        for task in token_tasks:
            ...
            if task["lemma"].lower() in HARDCODED_TRANSLATION_MAP:
                zh_val, aux_val = HARDCODED_TRANSLATION_MAP[task["lemma"].lower()]
                task["zh_trans"]  = zh_val
                task["aux_trans"] = aux_val
            elif ck_zh in app.my_dict and ck_aux in app.my_dict and "429" not in str(app.my_dict[ck_zh]) and "失败" not in str(app.my_dict[ck_zh]) and "重试" not in str(app.my_dict[ck_zh]) and app.my_dict[ck_zh] != task["lemma"]:
        ...
        for idiom in phrase_tasks:
            ...
            if ck_p in app.my_dict and "429" not in str(app.my_dict[ck_p]) and "重试" not in str(app.my_dict[ck_p]):
        ...
        if aux_query_lines:
            ...
            raw_aux_response = deepl_raw_translate(joined_aux_text, source_lang="en", target_lang=target_aux_code)
